...
function indekseGoreFenomen(dizi , i) {
  const secim = dizi[i]["profile"];
  return i + ". indekste bulunan fenomen: " + secim;
}
...
function fenomenEkle(dizi, no, isim, takipci, gonderi, yer) {
  const yeniDizi = [...dizi];
  const yeniFenomen = {number:no, profile:isim, followers:takipci, posts:gonderi, platform:yer};
...
function platformaGoreCokGonderiYapanFenomen(dizi, yer){
  ...
  let enCok = [0];
  for(let i=0; i<dizi.length; i++){
    if(dizi[i].platform.includes(yer) && dizi[i].posts>enCok){
      enCok = dizi[i].posts
